...
TEST_IMAGES = os.path.join(REPO, "devtools", "test_images")
RESULTS_PATH = os.path.join(REPO, "devtools", "tools", "regression_results.json")
...
def run_regression(reader=None):
    ...
    test_path = os.path.join("..", "CommandLineTests", "PokemonChampions")  # cwd=BUILD_DIR (switch_bot/build_mac); `..` = switch_bot/
    if reader:
...
def run_manifest_regression():
    ...
    gen_script = os.path.join(REPO, "devtools", "tools", "generate_test_registry.py")
    ...
    exe = os.path.join(BUILD_DIR, "SerialProgramsCommandLine")
    test_dir = os.path.join("..", "..", "devtools", "test_images")
