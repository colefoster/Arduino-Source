...
- **Pull the LiveDetectorTrace wiring into AutoLadder's per-turn loop.** Trace now feeds BattleLogReader, AbilityItemReader, CommunicatingDetector, TeamSelectDetector, TargetSelectDetector + Reader, **TeamSummaryReader + TeamStatsReader** (with dedup on the log/overlay readers). AutoLadder still calls a stripped-down subset.
- **Auto-load own team via TeamScan instead of OWN_TEAM_PASTE.** Trace now OCRs the View Details Moves & More + Stats tabs end-to-end (full team incl. nature + EVs, verified 6/6 clean). AutoLadder should navigate there once at session start in lieu of the paste workflow — the user no longer needs to maintain a Showdown paste.
...
- 2026-05-06 -- **Team Scan** (`/#/teamscan`). End-to-end visual OCR of the View Details Moves & More + Stats tabs. New `TeamStatsReader` (17 boxes per slot — actual/EVs/nature) + `TeamStatsTabDetector`. `TeamSummaryReader` extended with item field. Per-slot ABSOLUTE box table (cards aren't on a perfect grid → extrapolation lost ~5 fields per pass; per-slot hand-drawn = 0 misreads). `TrackedPokemon` gained `nature` + `evs[6]`; `update_from_team_summary` / `update_from_team_stats` merge methods skip empty/zero fields so partial reads don't clobber. Wired into LiveDetectorTrace cascade with screen-entry auto-capture. Dashboard tab shows side-by-side screenshots, polished per-slot cards (nature pill + ability/item line + 2x3 stat grid + move bullets), per-slot crop debugger, and the assembled Showdown paste at the bottom for copy-verify. 6/6 species/abilities/items/moves/natures correct on the test team. Digit-confusable fixup with `'a'→'9'` recovers Tesseract italic-stat misreads. See `memory/project_team_scan.md`.
- 2026-05-06 -- LiveDetectorTrace auto-capture. Off by default; ENABLE_AUTO_CAPTURE drops PNG + sidecar JSON into `test_images/_inbox/` for: under-labeled screen entries (target_select, team_select — once per match), novel BattleLogEventType (first per session), novel opp species (HUDReader), novel ability/item reveals. Sliding hourly cap. After a session, rsync the inbox up to ash and use the existing dashboard triage flow.
